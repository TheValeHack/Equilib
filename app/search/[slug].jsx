...
export default function SearchScreen() {
  ...
  const [currentPageNumber, setCurrentPageNumberSearch] = useState(1);
  const booksPerPage = 4;
  const totalPages = Math.ceil(data.filter(book =>
  ...
  const handleNextPage = () => {
    if (currentPageNumber < totalPages) {
      setCurrentPageNumberSearch(currentPageNumber + 1);
    }
  };
  ...
  useEffect(() => {
    setCurrentPageNumberSearch(1)
    dispatch({
      type: 'SET_EXTERNAL_DATA',
      payload: {
          externalData: {
              ...externalData,
              'setCurrentPageNumberSearch': setCurrentPageNumberSearch
          }
      }
    })
    ...
    <View className="bg-white min-h-[100%]">
      ...
      <ScrollView className="mt-4">
        ...
        <View className="flex flex-row flex-wrap justify-between w-full">
          ...
          {
            getCurrentPageBooks().length == 0 && <Text className="text-base mt-2" style={GlobalStyles.text_medium}>Tidak ditemukan buku yang sesuai dengan kata kunci "{slug}".</Text>
          }
        </View>
